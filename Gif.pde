...
class Gif {
  ...
  void show() {
    if (currentFrame == numFrames) currentFrame = 0; //loop gif once it reaches the end
    image(frames[currentFrame], x, y, w, h); //shows current frame
    if (frameCount % speed == 0) currentFrame++; //advance to next frame 
  }
